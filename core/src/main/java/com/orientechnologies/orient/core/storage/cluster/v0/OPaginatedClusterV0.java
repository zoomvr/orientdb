...
public final class OPaginatedClusterV0 extends OPaginatedCluster {
  ...
  public void open() throws IOException {
    ...
    try {
      ...
      final OCacheEntryImpl pinnedStateEntry = loadPageForRead(fileId, 0, false);
      try {
    ...
    try {
      ...
      try {
        ...
        do {
          ...
          boolean isNew = false;
          OCacheEntryImpl cacheEntry = loadPageForWrite(fileId, nextPageIndex, false, true);
          if (cacheEntry == null) {
  ...
  public long getEntries() {
    ...
    try {
      acquireSharedLock();
      try {
        final OCacheEntryImpl pinnedStateEntry = loadPageForRead(fileId, pinnedStateEntryIndex, true);
        try {
  ...
  private void updateClusterState(final long sizeDiff, final long recordsSizeDiff)
      throws IOException {
    final OCacheEntryImpl pinnedStateEntry = loadPageForWrite(fileId, pinnedStateEntryIndex, true, true);
    try {
  ...
  private byte[] readFullEntry(final long clusterPosition, long pageIndex, int recordPosition, final int pageCount)
      throws IOException {
    if (getFilledUpTo(fileId) <= pageIndex) {
      return null;
    }
  ...
  private FindFreePageResult findFreePage(final int contentSize) throws IOException {
    while (true) {
      ...
      if (freePageIndex < FREE_LIST_SIZE) {
        ...
        if (cacheEntry == null) {
          updateFreePagesList(freePageIndex, -1);
  ...
  private void initCusterState() throws IOException {
    final OCacheEntryImpl pinnedStateEntry = addPage(fileId);
    try {
      final OPaginatedClusterStateV0 paginatedClusterState = new OPaginatedClusterStateV0(pinnedStateEntry);
  ...
  public OClusterBrowsePage nextPage(final long lastPosition) throws IOException {
    ...
    try {
      acquireSharedLock();
      try {
        final OClusterPositionMapV0.OClusterPositionEntry[] nextPositions = clusterPositionMap.higherPositionsEntries(lastPosition);
