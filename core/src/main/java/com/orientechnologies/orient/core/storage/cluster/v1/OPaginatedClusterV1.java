...
public final class OPaginatedClusterV1 extends OPaginatedCluster {
        ...
        if (entryContentLength < OClusterPage.MAX_RECORD_SIZE) {
          ...
          do {
            ...
            if (prevPageRecordPointer >= 0) {
              ...
              final OCacheEntry prevPageCacheEntry = loadPageForWrite(fileId, prevPageIndex, false, true);
              try {
    ...
    try {
      ...
      try {
        ...
        do {
          ...
          if (isNew) {
            final OCacheEntry stateCacheEntry = loadPageForWrite(fileId, STATE_ENTRY_INDEX, false, true);
            try {
        ...
        while (nextEntryPointer >= 0) {
          ...
          final OCacheEntry cacheEntry = loadPageForWrite(fileId, nextPageIndex, false, true);
          try {
            final OClusterPage localPage = new OClusterPage(cacheEntry, false);
  ...
  public void setRecordConflictStrategy(final String conflictStrategy) {
    acquireExclusiveLock();
    try {
      recordConflictStrategy = Orient.instance().getRecordConflictStrategy().getStrategy(conflictStrategy);
    } finally {
      releaseExclusiveLock();
    }
  }
  ...
  @Override
  public void setEncryption(final String iMethod, final String iKey) {
    acquireExclusiveLock();
    try {
  ...
  private void updateFreePagesIndex(final int prevFreePageIndex, final long pageIndex) throws IOException {
    ...
    try {
      ...
      if (nextPageIndex >= 0) {
        final OCacheEntry nextPageCacheEntry = loadPageForWrite(fileId, nextPageIndex, false, true);
        try {
      ...
      if (newFreePageIndex >= 0) {
        ...
        if (oldFreePage >= 0) {
          final OCacheEntry oldFreePageCacheEntry = loadPageForWrite(fileId, oldFreePage, false, true);
          try {
