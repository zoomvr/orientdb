...
import static com.orientechnologies.orient.core.storage.impl.local.paginated.base.ODurablePage.MAX_PAGE_SIZE_BYTES;
 ...
 * @since 10/7/13
 */
public final class OFastRidbagClusterPositionMapBucket extends ODurablePage {
  ...
  private static final int ENTRY_SIZE = OByteSerializer.BYTE_SIZE + OIntegerSerializer.INT_SIZE + OLongSerializer.LONG_SIZE + OLongSerializer.LONG_SIZE + OLongSerializer.LONG_SIZE;

  public static final int  MAX_ENTRIES      = (MAX_PAGE_SIZE_BYTES - POSITIONS_OFFSET) / ENTRY_SIZE;

  public OFastRidbagClusterPositionMapBucket(OCacheEntry cacheEntry, boolean clear) {
    super(cacheEntry);
    if (clear) {
  ...
  public int add(long pageIndex, int recordPosition, long previousNodePosition, long nextNodePosition) {    
...
//  }
  ...
  private PositionEntry readEntry(int position) {
    ...
    final long previousNodePosition = getLongValue(position);
    position += OLongSerializer.LONG_SIZE;
    
    final long nextNodePosition = getLongValue(position);    

    return new PositionEntry(pageIndex, pagePosition, previousNodePosition, nextNodePosition);
  }

  private void updateEntry(int position, final PositionEntry entry) {
    position += OByteSerializer.BYTE_SIZE;

    position += setLongValue(position, entry.pageIndex);    
    position += setIntValue(position, entry.recordPosition);
    position += setLongValue(position, entry.previousNodePosition);
    position += setLongValue(position, entry.nextNodePosition);
  }
  ...
  public static final class PositionEntry {
    ...
    private final long previousNodePosition;
    private final long nextNodePosition;

    public PositionEntry(final long pageIndex,final  int recordPosition,
            final long previousNodePosition, final long nextNodePosition) {
